import time
import logging
log = logging.getLogger('zen.netAppVolumeDSP')
...
class NetAppVolumeDSP(PythonDataSourcePlugin, Utils):
    ...
    def collect(self, config):
          ...
          'volume': 
            dict.fromkeys(
              ...
              [
                ...
                ('read_latency', 'AVERAGE', 'read_ops'),
                ('write_latency', 'AVERAGE', 'write_ops'),
                ('read_data', 'RATE'),
                ('write_data', 'RATE'),
              ]
            )
        ...
        if naElements is None:
          returnValue(None)
        volumes = naElements[0].child_get('volumes')
        for volume in volumes.children_get():
          component = volume.child_get_string('name')
          results[ts][component]['size_used'] = volume.child_get_int('size-used')
          results[ts][component]['size_available'] = volume.child_get_int('size-available')
          results[ts][component]['percentage_used'] = volume.child_get_int('percentage-used')
    ...
    def onResult(self, result, config):
        ...
        for component in components:
          result[component] = {}
          result[component]['avg_latency'] = self._average(
            result[t2][component]['avg_latency'], result[t1][component]['avg_latency'],
            result[t2][component]['total_ops'], result[t1][component]['total_ops'])
          result[component]['read_latency'] = self._average(
            result[t2][component]['read_latency'], result[t1][component]['read_latency'],
            result[t2][component]['read_ops'], result[t1][component]['read_ops'])
          result[component]['write_latency'] = self._average(
            result[t2][component]['write_latency'], result[t1][component]['write_latency'],
            result[t2][component]['write_ops'], result[t1][component]['write_ops'])
          result[component]['read_data'] = (1./8) * self._rate(
            result[t2][component]['read_data'], result[t1][component]['read_data'],
            t2, t1)
          result[component]['write_data'] = (1./8) * self._rate(
            result[t2][component]['write_data'], result[t1][component]['write_data'],
            t2, t1)
    ...
    def onSuccess(self, result, config):
        ...
        for component in components:
          ...
          graphPoints['volume_avg_latency'] = (result[component]['avg_latency'], ts)
          graphPoints['volume_read_latency'] = (result[component]['read_latency'], ts)
          graphPoints['volume_write_latency'] = (result[component]['write_latency'], ts)
          graphPoints['volume_read_data'] = (result[component]['read_data'], ts)
          graphPoints['volume_write_data'] = (result[component]['write_data'], ts)
          graphPoints['volume_size_used'] = (result[component]['size_used'], ts)
          graphPoints['volume_size_available'] = (result[component]['size_available'], ts)
          graphPoints['volume_percentage_used'] = (result[component]['percentage_used'], ts)
    ...
    def onError(self, result, config):
        ...
        return {
            'events': [{
                ...
                'eventKey': 'netappvolume_datasource_result',
                'severity': 4,
                }],
            }
